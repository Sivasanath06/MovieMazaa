...
import Backup from "../assets/images/backup.avif"
...
export const MovieItem = () => {
  ...
  useEffect(() => {
    async function fetchMovie(){
      setLoading(true);
      const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=36a241e5a2245c977d3ff754861830e4`);
  ...
  const content=(<><div className="max-w-sm">
    <img className="rounded" src={image} alt={movie.title} />
  </div><div className="max-w-2xl text-gray-700 text-lg dark:text-white">
  ...
  return (
    <main>
      <section className="flex justify-around flex-wrap py-5">
        {loading ? <p className="text-7xl text-gray-700 font-bold my-10 dark:text-white">Loading...</p> : (
            content
        )}
